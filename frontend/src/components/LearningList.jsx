...
const LearningList = ({ sessions, onEdit, onDelete }) => {
  ...
  const filteredSessions = sessions.filter(session => {
    if (filters.subject && !session.subject.toLowerCase().includes(filters.subject.toLowerCase())) {
      return false;
    }
    ...
    <div className="card">
        ...
        <table className="w-full">
          ...
          <tbody>
            {filteredSessions.map((session) => (
              <tr key={session._id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800 dark:border-gray-700">
                <td className="py-3 px-4">
                  {format(new Date(session.date), 'MMM dd, yyyy')}
                </td>
                <td className="py-3 px-4 font-medium">{session.subject}</td>
                <td className="py-3 px-4">{session.topic}</td>
                <td className="py-3 px-4">{session.timeSpent} min</td>
                <td className="py-3 px-4">
      ...
      {filteredSessions.length === 0 && (
        <div className="text-center py-8 text-gray-500">
          No learning sessions found. Start tracking your learning!
        </div>
      )}
    </div>
  );
